...
| `customization_file`                            | (empty)                     | Path under `/config` to an optional YAML file extending the proxy's built-in scope knowledge. See `customization.example.yaml`.                                                                                         |
...
runtime, copy `customization.example.yaml` into `/config/` (renamed to
`dashboard_entity_proxy.yaml` or similar), point the `customization_file` option
at it, and add per-card implicit-entity entries. The file is fully additive;
built-in defaults for popular HACS cards stay in effect.
