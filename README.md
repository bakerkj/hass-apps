...
| [dashboard_entity_proxy/](dashboard_entity_proxy/) | Reverse-proxies the Home Assistant WebSocket per dashboard so slow tablets/kiosks only receive entity updates for the view they're showing. Resolves scope from Lovelace YAML, the entity/device/area registries, and (optionally) browser_mod. Ingress status UI shows per-client scope and queue depth. |
