...
| Entity                                          | Type            | Meaning                                                                                                                                                                                     |
| ----------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `binary_sensor.container_hooks_<slug>_applied`  | `binary_sensor` | `ON` iff `<container>/logs/pre-start.log` has a successful `put_archive` entry newer than the container's current `StartedAt`. Answers "did we fire the hook on this lifecycle."            |
