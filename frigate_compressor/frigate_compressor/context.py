...
class CompressorContext:
    ...
    # on the main loop thread.  Has Frigate's ``recordings`` attached as
    # ``frigate_eligible``.  Opened once at daemon startup so the per-iteration
    # eligibility query reuses a warm page cache instead of paying the schema
    # read + ATTACH cost every 60 s.  ``None`` in tests — eligibility then
    # falls back to opening a transient connection.
